/**
 *
 *  Class Generalization$Factory$Class.java
 *
 *  Generated by KMFStudio at 29 July 2003 10:22:57
 *  Visit http://www.cs.ukc.ac.uk/kmf
 *
 */

package uk.ac.ukc.cs.kmf.kmfstudio.uml.Foundation.Core;

import uk.ac.ukc.cs.kmf.kmfstudio.uml.UmlFactory$Class;
import uk.ac.ukc.cs.kmf.kmfstudio.uml.UmlVisitor;
import uk.ac.ukc.cs.kmf.kmfstudio.uml.repository.UmlRepository;

public class Generalization$Factory$Class
	extends UmlFactory$Class
	implements Generalization$Factory
{
	/** Default factory constructor */
	public Generalization$Factory$Class() {
	}
	public Generalization$Factory$Class(UmlRepository repository) {
		this.repository = repository;
	}

	/** Default build method */
	public Object build() {
		Generalization obj = new Generalization$Class();
		repository.addElement("uml.Foundation.Core.Generalization", obj);
		return obj;
	}
	/** Specialized build method */
	public Object build(uk.ac.ukc.cs.kmf.kmfstudio.uml.Foundation.Data_Types.Name name, uk.ac.ukc.cs.kmf.kmfstudio.uml.Foundation.Data_Types.VisibilityKind visibility, Boolean isSpecification, uk.ac.ukc.cs.kmf.kmfstudio.uml.Foundation.Data_Types.Name discriminator) {
		Generalization obj = new Generalization$Class(name, visibility, isSpecification, discriminator);
		repository.addElement("uml.Foundation.Core.Generalization", obj);
		return obj;
	}

	/** Override toString method */
	public String toString() {
		return "Generalization_Factory";
	}

	/** Accept 'uk.ac.ukc.cs.kmf.kmfstudio.uml.Foundation.Core.Generalization$Visitor' */
	public Object accept(UmlVisitor v, Object data) {
		return v.visit(this, data);
	}
}
