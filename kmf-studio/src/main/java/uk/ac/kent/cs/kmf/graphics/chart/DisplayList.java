...
/**
 *	A DisplayList is stored with other global information to record 
 *	information about each drawn object's position and shape.  By 
 *	passing a point and a Vector into the contains() method, you can 
 *	receive a list of all the objects that include the point.  Since 
 *	one location can contain many objects, this isn't as intuitive as 
 *	it may appear.  For example, a single point may correspond to an 
 *	individual bar (Datum), a set of bars (Dataset, Bar), a Plotarea, 
 *	and a Background.   Since the information is returned in your Vector 
 *	class as an anonymous list of objects, you can use the "instanceof" 
 *	operator to determine the object you're interested in.  
 *
 *	The Globals class access method setUseDisplayList 
 *	determines whether the display list should accumulate information 
 *	or not.   By default, the global display list is inactive.  Generally 
 *	each drawing class also contains an access 
 *	method setUseDisplayList, which lets you selectively eliminate some 
 *	classes from your list.  All drawing classes add information to the global
 *	display list by default.
 *
 *	If active, the global displayList is cleared each time a chart is 
 *	redrawn.
...
public class DisplayList implements Serializable {
	 ...
	 */
	public synchronized void addArc(
		Object obj,
		Point center,
		Point widthHeight,
		int startAngle,
		...
		xVal =
			center.x
				+ (int) (Math.cos(currentRadians + totalRadians)
					* (widthHeight.x / 2));
		yVal =
			center.y
				+ (int) (Math.sin(currentRadians + totalRadians)
					* (widthHeight.y / 2));
		...
		for (int i = 0; i < nPoints; i++) {
			xVal =
				center.x
					+ (int) (Math.cos(currentRadians) * (widthHeight.x / 2));
			yVal =
				center.y
					+ (int) (Math.sin(currentRadians) * (widthHeight.y / 2));
	...
	public void addLine(Object obj, Point start, Point end) {
		Point ll = new Point(start.x - 2, start.y);
		Point ur = new Point(end.x + 2, end.y);
	 ...
	 */
	public synchronized void addPolygon(
		Object obj,
		Point p[]) { //for area charts
		primitiveList.addElement(p);
		objectList.addElement(obj);
	}
	/**
	 ...
	 */
	public synchronized void addPolyline(
		Object obj,
	 ...
	 *@return       None
	 */
	public synchronized void addTextString(
		Object obj,
		int llx,
		int lly,
		String s,
	 ...
	 */
	public synchronized boolean contains(Point p, Vector yourVector) {
	...
	private boolean traverseList(Point p, Vector userVector) {
		Point myPoints[];
		Point pickPoint;
		int i;
