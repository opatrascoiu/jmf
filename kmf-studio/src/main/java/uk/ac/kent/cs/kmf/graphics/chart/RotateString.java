...
public class RotateString implements Serializable {
		...
		*/
		...
		for (int i = ny, index = 0, ii, ix, iy; i < nh + ny; i++) {
			for (int j = nx; j < nw + nx; j++) {
				ix =
					(int) (0.5
						+ (double) (j - ax) * Math.cos(-th)
						- (double) (i - ay) * Math.sin(-th)
						+ (double) ax);
				iy =
					(int) (0.5
						+ (j - ax) * Math.sin(-th)
						+ (double) (i - ay) * Math.cos(-th)
						+ (double) ay);
	...
	private Rectangle getExtent(
		...
		//required for buggy JVMs: rect.height = ascent + descent;
		rect.height = ascent;
	...
	private Rectangle rotateRectangle(
		...
		xmin =
			xmax =
				(double) (rect.x - ax) * Math.cos(th)
					- (double) (rect.y - ay) * Math.sin(th);
		ymin =
			ymax =
				(double) (rect.x - ax) * Math.sin(th)
					+ (double) (rect.y - ay) * Math.cos(th);
		fx =
			(double) (rect.x + rect.width - 1 - ax) * Math.cos(th)
				- (double) (rect.y - ay) * Math.sin(th);
		fy =
			(double) (rect.x + rect.width - 1 - ax) * Math.sin(th)
				+ (double) (rect.y - ay) * Math.cos(th);
		if (fx < xmin)
			xmin = fx;
		if (fx > xmax)
		...
		fx =
			(double) (rect.x - ax) * Math.cos(th)
				- (double) (rect.y + rect.height - 1 - ay) * Math.sin(th);
		fy =
			(double) (rect.x - ax) * Math.sin(th)
				+ (double) (rect.y + rect.height - 1 - ay) * Math.cos(th);
		if (fx < xmin)
			xmin = fx;
		if (fx > xmax)
			xmax = fx;
