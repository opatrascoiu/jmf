 ...
 *
 */
public class InvariantsParseAllInterfTemplate {
	public InvariantsParseAllInterfTemplate(Model model, Context context, Map invariants, PrintWriter out) {
		this.model = model;
	...
	/**
	  * Print ParseAll visitor interface
	  */
	public void generate() {
		// Print generation stamp
		context.getNaming().putStamp(context.getNaming().getAnalyseAllVisitorInterface(modelName), out);
		// Print the code
		out.println("package "+modelPackage+";");
		out.println();
		out.println("public interface "+context.getNaming().getParseAllVisitorInterface(modelName));
