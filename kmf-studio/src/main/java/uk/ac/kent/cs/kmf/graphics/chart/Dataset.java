...
/**
 *	A collection of Datum classes, along with a set of graphical 
 *	attributes (Gc) and labelling information.  In addition, Dataset 
 *	provides methods for determining certain metrics about the overall 
 *	collection.  These methods are used by some of the graphical 
 *	component classes (e.g. Pie percentages) as well as the Axis 
 *	classes (max, min, etc.)
 *
 *	Datasets are always initialized with a Gc, a dataset label color 
...
public class Dataset implements Serializable {
	 ...
	 *	KavaChart data feed beans, as all Chart datasets must include the Chart's
	 *	Globals class
	 */
	public Dataset() {
	...
	/***
	 * Constructor for HiLoClose charts.  Initializes everything but labels: 
	 * x, y, y2, and y3
	 * values.  Sets an initial Gc determined by the set number.
	 *@param        name    Dataset name
	 *@param        xarr    Array of X values
	 *@param        yarr    Array of Y values
	 *@param        y2arr   Array of Y2 values
	 *@param        y3arr   Array of Y3 values
	 *@param        setNumber	This Dataset's position in an array
	...
	/**
	 * Constructor for HiLo charts.  Initializes everything but labels: x, y, and y2
	 * values.  Sets an initial Gc determined by the set number.
	 *@param        name    Dataset name
	 *@param        xarr    Array of X values
	 *@param        yarr    Array of Y values
	...
	public Dataset(
		...
		for (int i = 0; i < yarr.length; i++) {
			data.addElement(
				new Datum(xarr[i], yarr[i], y2arr[i], setNumber, g));
		}
	}
	/**
	 *      Constructor for label charts that use both X and Y.  Sets initial
	 *      Gc determined by set number.  Also initializes labels for each data
	 *	point.
	 *@param        name    Dataset name
	 *@param        xarr    Array of X values
	 *@param        yarr    Array of Y values
	...
	public Dataset(
		...
		for (int i = 0; i < yarr.length; i++) {
			data.addElement(
				new Datum(xarr[i], yarr[i], label[i], setNumber, g));
		}
	}
	/**
	 *      Constructor for charts that use X and Y values, but have no labels.
	 *      Sets initial Gc determined by set number.
	 *@param        name    Dataset name
	 *@param        xarr    Array of X values
	 *@param        yarr    Array of Y values
	 *@param        setNumber	This Dataset's position in an array
	 ...
	 *@param        name    Dataset name
	 *@param        yarr    Array of chart data
	 *@param        label   Label values
	 *@param        setNumber	This Dataset's position in an array
	 *@param        g	This chart's Globals class
	 */
	public Dataset(
		...
		for (int i = 0; i < yarr.length; i++) {
			try {
				data.addElement(
					new Datum((double) i, yarr[i], label[i], setNumber, g));
			} catch (ArrayIndexOutOfBoundsException e) {
				data.addElement(
					new Datum((double) i, yarr[i], "", setNumber, g));
			}
		}
	}
	/**
	 *      Constructor for labelled charts that use only Y values.  Sets the
	 *      default initial Gc.
	 *@param        name    Dataset name
	 *@param        yarr    Array of chart data
	 *@param        label   Label values
	 *@param        individual      Use unique color for each data element's Gc
	 */
	public Dataset(
		String name,
		double yarr[],
		...
		for (int i = 0; i < yarr.length; i++) {
			try {
				data.addElement(
					new Datum((double) i, yarr[i], label[i], individual, g));
			} catch (ArrayIndexOutOfBoundsException e) {
	 ...
	 *	For example, this is appropriate for Pie charts.
	 *      Sets an initial Gc determined by set number.
	 *
	 *@param        name    Dataset name
	 *@param        yarr    Array of chart data
	 *@param        setNumber	This Dataset's position in an array
	 ...
	 *@param        name    Dataset name
	 *@param        yarr    Array of chart data
	 *@param        individual      Use unique or random color for each data element
	 *@param	g	this chart's Globals class
	 */
	...
	/**
	 * Installs a Datum class into this Dataset.  Also assigns this Dataset's Globals class
	 * to this Datum.
	 * @param d javachart.chart.Datum
	 */
	...
	/**
	 * Adds an observation to this Dataset.  Creates a new Datum class containing the
	 * X, Y, and label values specified.  Adds that class to the Dataset's data Vector.
	 * @param x double
	 * @param y double
	...
	public void dump() {
		...
		for (i = 0; i < data.size(); i++) {
			System.out.println(
				"X "
					+ ((Datum) data.elementAt(i)).x
					+ " Y "
					+ ((Datum) data.elementAt(i)).y
					+ " Y2 "
					+ ((Datum) data.elementAt(i)).y2);
		}
	}
	/**
	...
	/**
	 *	Returns this Dataset's Globals class, which is presumably the same as
	 *	the overall chart's Globals class.
	 */
	 ...
	 *      If the new array is smaller than the current array, the extra entries
	 *      at the end of the current array are not changed.
	 *@param        xarr    Array of new X values
	 */
	 ...
	 *      If the new array is smaller than the current array, the extra entries
	 *      at the end of the current array are not changed.
	 *@param        yarr    Array of new Y2 values
	 */
	 ...
	 *      If the new array is smaller than the current array, the extra entries
	 *      at the end of the current array are not changed.
	 *@param        yarr    Array of new Y3 values
	 */
	 ...
	 *      If the new array is smaller than the current array, the extra entries
	 *      at the end of the current array are not changed.
	 *@param        yarr    Array of new Y values
	 */
	...
	/**
	 *	sets this Dataset's Globals class, which is presumably the same as
	 *	the overall chart's Globals class.  Also installs this Globals class into all
