 ...
 *
 */
public class VisitableInterfTemplate {
	public VisitableInterfTemplate(Model model, Context context, PrintWriter out) {
		this.model = model;
	...
	/**
	  * Print Visitable interface
	  */
	public void generate() {
		// Print generation stamp
		context.getNaming().putStamp(context.getNaming().getVisitableInterface(modelName), out);
		// Print the code
		out.println("package "+modelPackage+";");
		out.println();
		out.println("public interface "+context.getNaming().getVisitableInterface(modelName));
		out.println("{");
		out.println(indent+"/** Accept the visitor */");
		out.println(indent+"public Object accept("+context.getNaming().getVisitorInterface(modelName)+" v, Object obj);");
		out.println("}");
	}
