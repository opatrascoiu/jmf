 ...
 * McCabe Complexity
 * 
 * Computes the McCabe metric for the OCL expression, 
 * considering OCL iterations as loop statements.
 *  
 */
public class OclMCCEvaluator
	extends Evaluator 
{
	public OclMCCEvaluator(QualityModel qModel) {
		super(qModel);
	}
	...
	protected void scanExp(OclExpressionAS exp) {
		...
		if (exp instanceof PrimaryExpAS) {
			if (exp instanceof TupleLiteralExp) {
				...
				// One node (literal) and n navigations
				nodes++;
				edges += tupleExp.getTuplePart().size();
			} else {
				// One node (Literal)
				nodes++;
			}
			...
			// One node (selected name) and one navigation
			edges++;
			nodes++;
			...
			// One node (test node) and one edge (goto test)
			nodes++;
			edges++;			
			...
			// One node (decision) and four edges
			nodes++;
			edges += 4;
