...
public class WsBrowser$Class extends JFrame {
	/** Construct the BROWSER */
	public WsBrowser$Class(String xmiFileName) {
		...
		//--- Set the repository ---
		setRep(new ws.repository.WsRepository$Class());
	...
	/** Repository */
	protected ws.repository.WsRepository crtRep;
	public void setRep(ws.repository.WsRepository rep) {
		//--- Reset console ---
		resetConsole();
	...
	protected void refreshViewDesktop() {
		...
		JInternalFrame frames[] = viewDesktop.getAllFrames();
		for(int i=0; i<frames.length; i++) ((WsViewEditFrame$Class)frames[i]).refreshAction();
	}
	...
	protected void refreshBuildDesktop() {
		...
		JInternalFrame frames[] = buildDesktop.getAllFrames();
		for(int i=0; i<frames.length; i++) ((WsInvokeMethod$Class)frames[i]).refreshAction();
	}
	...
	protected void newFileAction(ActionEvent e) {
		...
		crtRep = new ws.repository.WsRepository$Class();
	...
	protected void openFileAction(ActionEvent e) {
		...
		if (fileChooser.showOpenDialog(WsBrowser$Class.this) != JFileChooser.APPROVE_OPTION) return;
		//--- Get file name ---
		File file = fileChooser.getSelectedFile();
		crtFileName = file.getAbsolutePath();
		//--- Load the repository ---
		ws.repository.WsRepository$Class rep1 = null;
		if (crtFileName.endsWith("xmi")) {
			if (crtRep == null) crtRep = new ws.repository.WsRepository$Class(console);
			WsFactory$Class.resetId();
			rep1 = (ws.repository.WsRepository$Class)crtRep.loadXMI(crtFileName);
	...
	protected void buildEditAction(ActionEvent e) {
		...
		for(int i=0; i<selectedLifecycleNodes.size(); i++) {
			...
			if (!(factoryObject instanceof ws.WsFactory$Class)) continue;
			ws.WsFactory factory = (ws.WsFactory$Class)factoryObject;
			//--- Make Lifecycle frame ---
			WsInvokeMethod$Class buildFrame = new WsInvokeMethod$Class(crtRep, this, factory, method);
	...
	protected void deleteEditAction(ActionEvent e) {
		...
		for(int i=0; i<selectedElementNodes.size(); i++) {
			...
			if (!(userObject instanceof WsElement)) continue;
			WsElement element = (WsElement)userObject;
	...
	protected void classViewAction(ActionEvent e) {
		...
		for(int i=0; i<selectedElementNodes.size(); i++) {
			...
			WsElement element = (WsElement)userObject;
			//--- Make the VIEW frame ---
			WsViewVisitor view = new WsViewVisitor$Class(oclProcessor);
	...
	protected void classEditAction(ActionEvent e) {
		...
		for(int i=0; i<selectedElementNodes.size(); i++) {
			...
			WsElement element = (WsElement)userObject;
			//--- Make the EDIT frame ---
			WsEditVisitor edit = new WsEditVisitor$Class(oclProcessor);
...
//            UIManager.setLookAndFeel(new com.incors.plaf.alloy.AlloyLookAndFeel());
		...
		new WsBrowser$Class("C:/D/Work/Java Projects/YATL4KMF/src/test/scripts/WS.xmi");
	}
}
