/**
 *
 *  Class SyntaxBrowser$Class.java
 *
...
public class SyntaxBrowser$Class extends JFrame {
	/** Construct the BROWSER */
	public SyntaxBrowser$Class(String xmiFileName) {
		...
		//--- Set the repository ---
		setRep(new ocl.syntax.repository.SyntaxRepository$Class());
	...
	/** Repository */
	protected ocl.syntax.repository.SyntaxRepository crtRep;
	public void setRep(ocl.syntax.repository.SyntaxRepository rep) {
		//--- Reset console ---
		resetConsole();
	...
	protected JMenuBar createMenuBar() {
		...
		//--- Add ABOUT ---
		item = new JMenuItem("About syntax");
		item.setMnemonic('a');
		lst = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				aboutHelpAction(e);
	...
	protected void refreshViewDesktop() {
		...
		JInternalFrame frames[] = viewDesktop.getAllFrames();
		for(int i=0; i<frames.length; i++) ((SyntaxViewEditFrame$Class)frames[i]).refreshAction();
	}
	...
	protected void refreshBuildDesktop() {
		...
		JInternalFrame frames[] = buildDesktop.getAllFrames();
		for(int i=0; i<frames.length; i++) ((SyntaxInvokeMethod$Class)frames[i]).refreshAction();
	}
	...
	protected void newFileAction(ActionEvent e) {
		...
		crtRep = new ocl.syntax.repository.SyntaxRepository$Class();
	...
	protected void openFileAction(ActionEvent e) {
		...
		if (fileChooser.showOpenDialog(SyntaxBrowser$Class.this) != JFileChooser.APPROVE_OPTION) return;
		//--- Get file name ---
		File file = fileChooser.getSelectedFile();
		crtFileName = file.getAbsolutePath();
		//--- Load the repository ---
		ocl.syntax.repository.SyntaxRepository$Class rep1 = null;
		if (crtFileName.endsWith("xmi")) {
			if (crtRep == null) crtRep = new ocl.syntax.repository.SyntaxRepository$Class(console);
			SyntaxFactory$Class.resetId();
			rep1 = (ocl.syntax.repository.SyntaxRepository$Class)crtRep.loadXMI(crtFileName);
	...
	protected void buildEditAction(ActionEvent e) {
		...
		for(int i=0; i<selectedLifecycleNodes.size(); i++) {
			...
			if (!(factoryObject instanceof SyntaxFactory$Class)) continue;
			SyntaxFactory factory = (SyntaxFactory$Class)factoryObject;
			//--- Make Lifecycle frame ---
			SyntaxInvokeMethod$Class buildFrame = new SyntaxInvokeMethod$Class(crtRep, this, factory, method);
	...
	protected void deleteEditAction(ActionEvent e) {
		...
		for(int i=0; i<selectedElementNodes.size(); i++) {
			...
			if (!(userObject instanceof SyntaxElement)) continue;
			SyntaxElement element = (SyntaxElement)userObject;
	...
	protected void classViewAction(ActionEvent e) {
		...
		for(int i=0; i<selectedElementNodes.size(); i++) {
			...
			SyntaxElement element = (SyntaxElement)userObject;
			//--- Make the VIEW frame ---
			SyntaxViewVisitor view = new SyntaxViewVisitor$Class(oclProcessor);
	...
	protected void classEditAction(ActionEvent e) {
		...
		for(int i=0; i<selectedElementNodes.size(); i++) {
			...
			SyntaxElement element = (SyntaxElement)userObject;
			//--- Make the EDIT frame ---
			SyntaxEditVisitor edit = new SyntaxEditVisitor$Class(oclProcessor);
...
//            UIManager.setLookAndFeel(new com.incors.plaf.alloy.AlloyLookAndFeel());
		...
		new SyntaxBrowser$Class("C:/D/Work/Java Projects/YATL4KMF/src/test/scripts/ocl.xmi");
	}
}
