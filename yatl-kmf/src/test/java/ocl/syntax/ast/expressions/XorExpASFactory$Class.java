/**
 *
 *  Class XorExpASFactory$Class.java
 *
 *  Generated by KMFStudio at 17 February 2004 14:38:43
 *  Visit http://www.cs.ukc.ac.uk/kmf
 *
 */

package ocl.syntax.ast.expressions;

public class XorExpASFactory$Class
	extends ocl.syntax.SyntaxFactory$Class
	implements XorExpASFactory
{
	/** Default factory constructor */
	public XorExpASFactory$Class() {
	}
	public XorExpASFactory$Class(ocl.syntax.repository.SyntaxRepository repository) {
		this.repository = repository;
	}

	/** Default build method */
	public Object build() {
		XorExpAS obj = new XorExpAS$Class();
		obj.setId(ocl.syntax.SyntaxFactory$Class.newId());
		repository.addElement("syntax.ast.expressions.XorExpAS", obj);
		return obj;
	}
	/** Specialized build method */
	public Object build(Boolean isMarkedPre) {
		XorExpAS obj = new XorExpAS$Class(isMarkedPre);
		obj.setId(ocl.syntax.SyntaxFactory$Class.newId());
		repository.addElement("syntax.ast.expressions.XorExpAS", obj);
		return obj;
	}

	/** Override toString method */
	public String toString() {
		return "XorExpASFactory";
	}

	/** Accept 'ocl.syntax.ast.expressions.XorExpASVisitor' */
	public Object accept(ocl.syntax.SyntaxVisitor v, Object data) {
		return v.visit(this, data);
	}
}
