...
public class SdBrowser$Class extends JFrame {
	/** Construct the BROWSER */
	public SdBrowser$Class(String xmiFileName) {
		...
		//--- Set the repository ---
		setRep(new sd.repository.SdRepository$Class());
	...
	/** Repository */
	protected sd.repository.SdRepository crtRep;
	public void setRep(sd.repository.SdRepository rep) {
		//--- Reset console ---
		resetConsole();
	...
	protected void refreshViewDesktop() {
		...
		JInternalFrame frames[] = viewDesktop.getAllFrames();
		for(int i=0; i<frames.length; i++) ((SdViewEditFrame$Class)frames[i]).refreshAction();
	}
	...
	protected void refreshBuildDesktop() {
		...
		JInternalFrame frames[] = buildDesktop.getAllFrames();
		for(int i=0; i<frames.length; i++) ((SdInvokeMethod$Class)frames[i]).refreshAction();
	}
	...
	protected void newFileAction(ActionEvent e) {
		...
		crtRep = new sd.repository.SdRepository$Class();
	...
	protected void openFileAction(ActionEvent e) {
		...
		if (fileChooser.showOpenDialog(SdBrowser$Class.this) != JFileChooser.APPROVE_OPTION) return;
		//--- Get file name ---
		File file = fileChooser.getSelectedFile();
		crtFileName = file.getAbsolutePath();
		//--- Load the repository ---
		sd.repository.SdRepository$Class rep1 = null;
		if (crtFileName.endsWith("xmi")) {
			if (crtRep == null) crtRep = new sd.repository.SdRepository$Class(console);
			SdFactory$Class.resetId();
			rep1 = (sd.repository.SdRepository$Class)crtRep.loadXMI(crtFileName);
	...
	protected void buildEditAction(ActionEvent e) {
		...
		for(int i=0; i<selectedLifecycleNodes.size(); i++) {
			...
			if (!(factoryObject instanceof SdFactory$Class)) continue;
			SdFactory factory = (SdFactory$Class)factoryObject;
			//--- Make Lifecycle frame ---
			SdInvokeMethod$Class buildFrame = new SdInvokeMethod$Class(crtRep, this, factory, method);
	...
	protected void deleteEditAction(ActionEvent e) {
		...
		for(int i=0; i<selectedElementNodes.size(); i++) {
			...
			if (!(userObject instanceof SdElement)) continue;
			SdElement element = (SdElement)userObject;
	...
	protected void classViewAction(ActionEvent e) {
		...
		for(int i=0; i<selectedElementNodes.size(); i++) {
			...
			SdElement element = (SdElement)userObject;
			//--- Make the VIEW frame ---
			SdViewVisitor view = new SdViewVisitor$Class(oclProcessor);
	...
	protected void classEditAction(ActionEvent e) {
		...
		for(int i=0; i<selectedElementNodes.size(); i++) {
			...
			SdElement element = (SdElement)userObject;
			//--- Make the EDIT frame ---
			SdEditVisitor edit = new SdEditVisitor$Class(oclProcessor);
...
//            UIManager.setLookAndFeel(new com.incors.plaf.alloy.AlloyLookAndFeel());
		...
		new SdBrowser$Class("C:/D/Work/Java Projects/YATL4KMF/src/test/scripts/ECA.xmi");
	}
}
