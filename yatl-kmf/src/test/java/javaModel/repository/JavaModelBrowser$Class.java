/**
 *
 *  Class JavaModelBrowser$Class.java
 *
...
public class JavaModelBrowser$Class extends JFrame {
	/** Construct the BROWSER */
	public JavaModelBrowser$Class(String xmiFileName) {
		...
		//--- Set the repository ---
		setRep(new javaModel.repository.JavaModelRepository$Class());
	...
	/** Repository */
	protected javaModel.repository.JavaModelRepository crtRep;
	public void setRep(javaModel.repository.JavaModelRepository rep) {
		//--- Reset console ---
		resetConsole();
	...
	protected JMenuBar createMenuBar() {
		...
		//--- Add ABOUT ---
		item = new JMenuItem("About javaModel");
		item.setMnemonic('a');
		lst = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				aboutHelpAction(e);
	...
	protected void refreshViewDesktop() {
		...
		JInternalFrame frames[] = viewDesktop.getAllFrames();
		for(int i=0; i<frames.length; i++) ((JavaModelViewEditFrame$Class)frames[i]).refreshAction();
	}
	...
	protected void refreshBuildDesktop() {
		...
		JInternalFrame frames[] = buildDesktop.getAllFrames();
		for(int i=0; i<frames.length; i++) ((JavaModelInvokeMethod$Class)frames[i]).refreshAction();
	}
	...
	protected void newFileAction(ActionEvent e) {
		...
		crtRep = new javaModel.repository.JavaModelRepository$Class();
	...
	protected void openFileAction(ActionEvent e) {
		...
		if (fileChooser.showOpenDialog(JavaModelBrowser$Class.this) != JFileChooser.APPROVE_OPTION) return;
		//--- Get file name ---
		File file = fileChooser.getSelectedFile();
		crtFileName = file.getAbsolutePath();
		//--- Load the repository ---
		javaModel.repository.JavaModelRepository$Class rep1 = null;
		if (crtFileName.endsWith("xmi")) {
			if (crtRep == null) crtRep = new javaModel.repository.JavaModelRepository$Class(console);
			JavaModelFactory$Class.resetId();
			rep1 = (javaModel.repository.JavaModelRepository$Class)crtRep.loadXMI(crtFileName);
	...
	protected void buildEditAction(ActionEvent e) {
		...
		for(int i=0; i<selectedLifecycleNodes.size(); i++) {
			...
			if (!(factoryObject instanceof javaModel.JavaModelFactory$Class)) continue;
			javaModel.JavaModelFactory factory = (javaModel.JavaModelFactory$Class)factoryObject;
			//--- Make Lifecycle frame ---
			JavaModelInvokeMethod$Class buildFrame = new JavaModelInvokeMethod$Class(crtRep, this, factory, method);
	...
	protected void deleteEditAction(ActionEvent e) {
		...
		for(int i=0; i<selectedElementNodes.size(); i++) {
			...
			if (!(userObject instanceof JavaModelElement)) continue;
			JavaModelElement element = (JavaModelElement)userObject;
	...
	protected void classViewAction(ActionEvent e) {
		...
		for(int i=0; i<selectedElementNodes.size(); i++) {
			...
			JavaModelElement element = (JavaModelElement)userObject;
			//--- Make the VIEW frame ---
			JavaModelViewVisitor view = new JavaModelViewVisitor$Class(oclProcessor);
	...
	protected void classEditAction(ActionEvent e) {
		...
		for(int i=0; i<selectedElementNodes.size(); i++) {
			...
			JavaModelElement element = (JavaModelElement)userObject;
			//--- Make the EDIT frame ---
			JavaModelEditVisitor edit = new JavaModelEditVisitor$Class(oclProcessor);
...
//            UIManager.setLookAndFeel(new com.incors.plaf.alloy.AlloyLookAndFeel());
		...
		new JavaModelBrowser$Class("C:/D/Work/Java Projects/YATL4KMF/src/test/scripts/java.xmi");
	}
}
